...
> 클래스간 공통 메소드, 혹 필드들을 공유 하는 abstract
...
>List.anyList 라는 가상의 메소드가 있는데, 리턴타입이 List라고 가정

>세부적인 내용들을 몰라도 

> List라는 기본적인 interface의 사용목적, 사용방법들을 알고있는 우리 A

> List.anyList에서 돌려줄 어떤 List를 만들 B

> 우리는 무리없이 저 메소드에서 돌려받을 List가 어떤 인스턴스인지는 몰라도 사용가능함.
